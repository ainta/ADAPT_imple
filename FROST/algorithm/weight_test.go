...
func virtualizedSign(p VirtualizedParticipant, totalParticipants, threshold int) {
	...
	for _, vId := range p.VirtualIds {
		...
		go func(idx int) {
			...
			lagrange, _ := ed.NewScalar().SetUniformBytes(makeRandomBytes(64))

			// Calculate response values
			bytes := makeRandomBytes(64)
			z, _ := ed.NewScalar().SetUniformBytes(bytes)
		}(vId)
	}
	wg.Wait()
}
...
func main() {
	if len(os.Args) > 1 && os.Args[1] == "weight-test" {
		...
		fmt.Println("This file is for weight testing only.")
		fmt.Println("To run: go run weight_test.go weight-test")
	}
}
